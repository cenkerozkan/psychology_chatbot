...
async def event_generator(
    ...
    logger.info(f"Starting stream for chat_id: {chat_thread.chat_id}")
    ...
    logger.debug(f"Sending message to chat service: '{chat_request.message[:50]}...'")
    ...
    if isinstance(full_response, dict) and 'content' in full_response:
        ...
        try:
            ...
            logger.warning(f"Failed to parse response as dict: {full_response[:100]}...")
    ...
    logger.info(f"Stream completed for chat_id: {chat_thread.chat_id}")
...
async def send_message_stream(
    ...
    """
    logger.info(f"Stream request received with chat_id: {chat_request.chat_id}")
    chat_thread = None

    # Use existing chat if chat_id is provided
    if chat_request.chat_id:
        logger.debug(f"Looking up existing chat with ID: {chat_request.chat_id}")
        chat_thread = await context_repository.get_one_by_id(chat_request.chat_id)
        if not chat_thread:
            logger.warning(f"Chat thread not found: {chat_request.chat_id}")
    ...
    if not chat_thread:
        ...
        logger.info(f"Creating new chat thread with ID: {chat_id}")
        chat_thread = ChatThreadModel(
            chat_id=chat_id,
            chat_name=chat_name,
            created_at=now,
...
async def send_message(
    ...
    """
    logger.info(f"Message request received with chat_id: {chat_request.chat_id}")
    chat_thread = None
...
async def create_daily_chat(api_key: APIKey = Depends(get_api_key)):
    ...
    """
    logger.info("Creating new daily chat thread")
    today = datetime.datetime.now().strftime('%Y-%m-%d')
    chat_name = f"Chat_{today}"
    chat_id = str(uuid.uuid4())
    now = datetime.datetime.now().isoformat()
    ...
    # Save the new chat thread
    await context_repository.insert_one(chat_thread)
    logger.info(f"New daily chat created with ID: {chat_id}")
...
async def get_all_chats(api_key: APIKey = Depends(get_api_key)):
    ...
    logger.debug(f"Found {len(threads)} chat threads")
...
async def get_chat_history(
    ...
    """
    logger.info(f"Retrieving message history for chat_id: {chat_id}")
    chat_thread = await context_repository.get_one_by_id(chat_id)
    if not chat_thread:
        logger.warning(f"Chat thread not found: {chat_id}")
        raise HTTPException(status_code=404, detail=f"Chat thread with ID {chat_id} not found")

    logger.debug(f"Found {len(chat_thread.history)} messages in chat_id: {chat_id}")
    return [
        {
            "role": msg.role,
            "content": msg.content,
...
async def delete_chat(chat_id: str, api_key: APIKey = Depends(get_api_key)):
    ...
    """
    logger.info(f"Deleting chat thread with ID: {chat_id}")
    chat_thread = await context_repository.get_one_by_id(chat_id)
    if not chat_thread:
        logger.warning(f"Chat thread not found: {chat_id}")
        raise HTTPException(status_code=404, detail=f"Chat thread with ID {chat_id} not found")
    ...
    if not result:
        logger.error(f"Failed to delete chat thread: {chat_id}")
        raise HTTPException(status_code=500, detail="Failed to delete chat thread")

    logger.info(f"Successfully deleted chat thread: {chat_id}")
